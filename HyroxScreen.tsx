...
const RaceEditor: React.FC<{
    ...
    return (
        <div className="w-full max-w-4xl mx-auto animate-fade-in text-left">
            <h1 className="text-4xl lg:text-5xl font-extrabold text-white mb-6">Redigera race</h1>
            <div className="bg-gray-800 rounded-xl p-6 border border-gray-700 space-y-6">
                <div>
                    <label htmlFor="race-name" className="block text-sm font-medium text-gray-400 mb-1">Racets namn</label>
                    <input
                        id="race-name"
                        type="text"
                        value={raceName}
                        onChange={(e) => setRaceName(e.target.value)}
                        className="w-full bg-black/50 text-white placeholder-gray-400 p-3 rounded-md border border-gray-600 focus:ring-1 focus:ring-primary focus:outline-none"
                    />
                </div>

                <div>
                    <h3 className="text-lg font-semibold text-white mb-2">Stationer</h3>
                    <div className="space-y-2 max-h-[50vh] overflow-y-auto pr-2">
                        {exercises.map((ex, index) => (
                            <div
                                key={ex.id || index}
                                className="flex items-center gap-2 p-2 bg-gray-900/50 rounded-md border border-gray-700"
                                draggable
                            ...
                            >
                                <div className="cursor-grab text-gray-500 hover:text-white p-2">☰</div>
                                ...
                                <input
                                    ...
                                    className="w-32 bg-transparent text-white focus:outline-none placeholder-gray-400 p-1 rounded"
                                />
                                <input
                                    type="text"
                                    value={ex.name || ''}
                                    ...
                                    className="flex-grow bg-transparent text-white focus:outline-none placeholder-gray-400 p-1 rounded"
                                />
                ...
                <button onClick={handleAddExercise} className="w-full text-center py-2 border-2 border-dashed border-gray-600 rounded-lg text-gray-400 hover:bg-gray-700 hover:border-gray-500 transition-colors">
                    + Lägg till station
                </button>
            </div>
            <div className="mt-6 flex gap-4">
                <button onClick={onCancel} className="flex-1 bg-gray-600 hover:bg-gray-500 text-white font-bold py-3 rounded-lg">Avbryt</button>
...
const StartGroupPrepModal: React.FC<{
    ...
    return (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={onClose}>
            <div className="bg-gray-800 rounded-xl p-6 sm:p-8 w-full max-w-2xl h-[90vh] text-white shadow-2xl border border-gray-700 animate-fade-in flex flex-col" onClick={e => e.stopPropagation()}>
                <h2 className="text-3xl font-bold mb-6 flex-shrink-0">Förbered Startgrupper</h2>
                
                <div className="flex-grow overflow-y-auto space-y-4 pr-2">
                    {groups.map((group) => (
                        <div key={group.id} className="bg-gray-900/50 p-4 rounded-lg border border-gray-700">
                            <div className="flex justify-between items-center mb-2">
                                <input
                                    type="text"
                                    value={group.name}
                                    onChange={(e) => handleGroupChange(group.id, 'name', e.target.value)}
                                    className="bg-transparent font-semibold text-white focus:outline-none p-1 text-lg"
                                />
                                {groups.length > 1 && (
                            ...
                            <textarea
                                ...
                                placeholder="Deltagare (ett namn per rad)"
                                rows={4}
                                className="w-full bg-black/50 text-base text-gray-300 p-2 rounded-md border border-gray-600 focus:ring-1 focus:ring-primary focus:outline-none resize-y"
                            />
                        </div>
                    ))}
                </div>
                ...
                <div className="flex-shrink-0 pt-4">
                     <button onClick={handleAddGroup} className="w-full bg-gray-700 hover:bg-gray-600 text-white font-semibold py-3 rounded-lg transition-colors mb-4">
                        + Lägg till startgrupp
                    </button>
                    <div className="my-4 text-center">
                        <label htmlFor="start-interval" className="block text-sm font-medium text-gray-400 mb-1">
                        ...
                        <div className="flex items-center justify-center gap-2">
                             <input
                                ...
                                min="1"
                                className="w-24 bg-black/50 text-white text-center p-2 rounded-md border border-gray-600 focus:ring-1 focus:ring-primary focus:outline-none"
                            />
                            <span className="text-gray-400">minuter</span>
                        </div>
                    </div>
                    <div className="flex flex-col sm:flex-row gap-4 mt-6">
                        ...
                        <button onClick={handleStartRace} className="flex-1 bg-primary hover:brightness-95 text-white font-bold py-3 rounded-lg transition-colors">Starta Loppet</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    return (
        <div className="w-full max-w-5xl mx-auto text-center animate-fade-in">
            <p className="text-lg text-gray-500 dark:text-gray-400 mb-10 max-w-2xl mx-auto">
                Kör hela HYROX-loppet som en tävling – från första löpningen till sista repetitionen.
            </p>
            <div className="flex flex-col items-center gap-6">
                <button
                    onClick={handleSimulateFullRaceClick}
                    className="bg-gray-800 hover:bg-gray-700 text-white p-6 rounded-xl transition-colors duration-200 flex flex-col items-center justify-center shadow-lg border border-gray-700 h-72 w-full max-w-lg"
                >
                    <h3 className="text-4xl font-extrabold text-primary">🏁 Simulera Hela Loppet</h3>
                    <p className="text-lg font-normal text-gray-400 mt-4">Kör hela loppet "For Time" i ett enda svep.</p>
                </button>

                <button
                    onClick={() => navigateTo(Page.HyroxRaceList)}
                    className="w-full max-w-lg bg-gray-800/50 hover:bg-gray-700 text-white font-semibold py-4 px-6 rounded-xl transition-colors border border-gray-700"
                >
                    Visa tidigare lopp
                </button>
            </div>
