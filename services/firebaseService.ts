...
import { Studio, StudioConfig, Organization, CustomPage, UserData, Workout, InfoCarousel, DisplayConfig } from '../types';
...
export const updateOrganizationDisplayConfig = async (organizationId: string, displayConfig: DisplayConfig): Promise<Organization> => {
    if(isOffline || !db) {
        await offlineWarning('updateOrganizationDisplayConfig');
        const org = MOCK_ORGANIZATIONS.find(o => o.id === organizationId);
        if (org) {
            org.displayConfig = displayConfig;
        }
        return getUpdatedOrg(organizationId);
    }
    await db.collection('organizations').doc(organizationId).update({ displayConfig });
    return getUpdatedOrg(organizationId);
};
...
export const inviteUser = async (organizationId: string, email: string, role: 'coach' | 'admin'): Promise<{success: boolean, message: string, link?: string}> => {
    ...
    // FIX: Switched to compat API for Firebase Functions
    try {
        if (!app) {
