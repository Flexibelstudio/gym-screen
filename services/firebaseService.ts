...
export const getPastRaces = async (organizationId: string): Promise<HyroxRace[]> => {
    ...
    // Removed orderBy("createdAt", "desc") to avoid need for composite index
    const q = racesCol.where("organizationId", "==", organizationId);
    const querySnapshot = await q.get();
    const races = querySnapshot.docs.map(doc => doc.data() as HyroxRace);
    // Client-side sort
