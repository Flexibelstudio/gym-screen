...
export async function generateWorkout(userPrompt: string): Promise<Workout> {
    const fullPrompt = `
        ...
        **Timer Settings (Crucial!):**
        ...
        - **Ingen Timer**: If a block is purely for strength with sets and reps (e.g., '5x5 Squats'), and no time is specified, use the 'Ingen Timer' mode. For this mode, \`workTime\` MUST be 0, \`restTime\` MUST be 0, and \`rounds\` MUST be 1.
    ...
    const fullPrompt = `
        ...
        **Parsing Rules & Logic:**
        ...
        3.  **Identify Timer Settings**: Look for clues like "AMRAP 15 min", "EMOM 10", "30/15", "4 rounds", "Tabata". If no timer info is given, choose a suitable default based on the block's purpose.
            ...
            - If the text describes a block with exercises and reps but no timing information (e.g., just '3x10 Bicep Curls'), infer that it should use 'Ingen Timer' mode. For this mode, set workTime to 0, restTime to 0, and rounds to 1.
