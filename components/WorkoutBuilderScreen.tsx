...
import { Workout, WorkoutBlock, Exercise, TimerMode, TimerSettings, Passkategori, StudioConfig, UserRole, BankExercise } from '../types';
import { ToggleSwitch, DumbbellIcon } from './icons';
import { TimerSetupModal } from './TimerSetupModal';
import { getExerciseBank, uploadImage, deleteImageByUrl } from '../services/firebaseService';
                ...
                {isLoading ? (
                    ...
                    filteredBank.map(ex => (
                        <div key={ex.id} className="bg-white dark:bg-gray-900/70 rounded-md p-2 flex items-center gap-3">
                            ...
                            >
                                {ex.imageUrl ? (
                                    <img src={ex.imageUrl} alt={ex.name} className="w-16 h-16 object-cover rounded flex-shrink-0 bg-gray-700"/>
                                ) : (
        ...
        >
            ...
            >
                ...
                <div className="flex-grow overflow-y-auto pr-2 space-y-4">
                    {exercise.imageUrl ? (
                        <img src={exercise.imageUrl} alt={exercise.name} className="w-full h-64 object-cover rounded-lg bg-gray-700"/>
                    ) : (
...
export const WorkoutBuilderScreen: React.FC<WorkoutBuilderScreenProps> = ({ initialWorkout, onSave, onCancel, focusedBlockId: initialFocusedBlockId, studioConfig, sessionRole, isNewDraft = false }) => {
  ...
  const handleAddExerciseFromBank = (bankExercise: BankExercise) => {
    ...
    const newExercise: Exercise = {
        ...
        reps: '', // Let the coach fill this in
        isFromBank: true,
    };
...
const EditableBlockCard: React.FC<EditableBlockCardProps> = ({ block, onUpdate, onRemove, onEditSettings, isDraggable, workoutTitle, workoutBlocksCount, editorRefs, exerciseBank, organizationId }) => {
...
const ExerciseImageUploader: React.FC<ExerciseImageUploaderProps> = ({ imageUrl, onImageChange, organizationId }) => {
  ...
  const handleFile = async (file: File | null) => {
    if (file && file.type.startsWith('image/')) {
        ...
        try {
            ...
            const path = `organizations/${organizationId}/exercises/${Date.now()}-${file.name}`;
      ...
      {imageUrl ? (
        ...
        <div className="space-y-2">
            <input
                ...
                className="w-full bg-white dark:bg-gray-700 text-gray-800 dark:text-white text-sm p-2 rounded-md border border-gray-300 dark:border-gray-600 focus:ring-1 focus:ring-primary focus:outline-none"
            />
            ...
            <div
              ...
              className={`relative flex flex-col items-center justify-center p-4 h-24 border-2 border-dashed rounded-lg cursor-pointer transition-colors ${
                isDragging ? 'border-primary bg-primary/20' : 'border-gray-400 dark:border-gray-600 hover:border-primary hover:bg-gray-100 dark:hover:bg-gray-700/50'
              }`}
            >
...
const EditableExerciseItem: React.FC<EditableExerciseItemProps> = ({ exercise, onChange, onRemove, exerciseBank, organizationId }) => {
    ...
    const handleImageChange = (newUrl: string) => {
        if (exercise.imageUrl && newUrl !== exercise.imageUrl) {
            deleteImageByUrl(exercise.imageUrl);
        }
    ...
    return (
        <div 
          ...
          className="group p-3 rounded-lg flex items-start gap-2 transition-all bg-gray-100 dark:bg-gray-700/50 relative"
        >
                    ...
                    {imageUploaderVisible ? (
                        <div className="animate-fade-in">
                            <div className="flex justify-between items-center mb-2">
                                ...
                                <button onClick={() => setImageUploaderVisible(false)} className="text-xs font-semibold text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white">
                                    Dölj
                                </button>
                            </div>
                        ...
                        <button onClick={() => setImageUploaderVisible(true)} className="flex items-center gap-2 text-sm font-semibold text-primary hover:underline">
                            <span>{exercise.imageUrl ? 'Visa/Byt bild' : 'Lägg till bild'}</span>
                        </button>
                    )}
                </div>
            </div>
