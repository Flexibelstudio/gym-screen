...
const WARMUP_PROMPT = `
Du är en expertcoach på Flexibel Hälsostudio. Ditt uppdrag är att skapa en dynamisk och varierad helkroppsuppvärmning. Passen ska vara mellan 3 och 5 minuter långa totalt, och i de flesta fall utan vila mellan övningarna.
...
*   **VARIATION:** Skapa variation genom att välja OLIKA upplägg varje gång. Här är några exempel på strukturer som du kan använda och anpassa för att landa på 3-5 minuter. **Välj ett nytt upplägg varje gång du genererar ett pass!**
    *   **Struktur 1 (Klassisk cirkel):** 10 övningar, 30 sekunders arbete per övning, 1 varv. (Totalt 5 min)
    *   **Struktur 2 (Kort & intensiv):** 6 övningar, 30 sekunders arbete per övning, 1 varv. (Totalt 3 min)
    *   **Struktur 3 (Två varv):** 5 övningar, 30 sekunders arbete per övning, 2 varv. (Totalt 5 min)
    *   **Struktur 4 (Längre intervaller):** 5 övningar, 45 sekunders arbete per övning, 1 varv. (Totalt ~3.75 min)
    *   **Struktur 5 (Stege):** En serie med 3-4 övningar som repeteras i flera varv, t.ex. 4 övningar x 20 sek x 3 varv (Totalt 4 min).
*   **Konsistens:** Se till att värdena i \`settings\` (workTime, restTime, rounds) och antalet övningar i \`exercises\`-listan exakt matchar det upplägg du valt.
`;
...
export const WarmupScreen: React.FC<WarmupScreenProps> = ({ onStartBlock }) => {
  ...
  const settingsText = useMemo(() => {
    ...
    const desc = `${settings.rounds} varv, ${exercises.length} övningar`;
    return `${time} • ${desc}`;
  }, [warmupBlock]);
    ...
    return (
        <div className="w-full max-w-2xl mx-auto animate-fade-in space-y-6">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
                <button
                    onClick={() => onStartBlock(warmupBlock)}
