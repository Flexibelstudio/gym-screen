...
*   **'rounds'-fältet:** För \`"Intervall"\`-läget representerar fältet \`rounds\` det **TOTALA ANTALET ARBETSINTERVALLER**. Det är INTE antalet varv.
*   **VARIATION:** Skapa variation genom att välja OLIKA upplägg varje gång. Här är några exempel på strukturer som du kan använda och anpassa för att landa på 3-5 minuter. **Välj ett nytt upplägg varje gång du genererar ett pass!**
    *   **Struktur 1 (Klassisk cirkel):** 10 övningar, 30s arbete/övning, 1 varv. Detta innebär 10 totala intervaller. Sätt \`rounds\` till 10.
    *   **Struktur 2 (Kort & intensiv):** 6 övningar, 30s arbete/övning, 1 varv. Detta innebär 6 totala intervaller. Sätt \`rounds\` till 6.
    *   **Struktur 3 (Två varv):** 5 övningar, 30s arbete/övning, 2 varv. Detta innebär 10 totala intervaller (5 övn * 2 varv). Sätt \`rounds\` till 10.
    *   **Struktur 4 (Längre intervaller):** 5 övningar, 45s arbete/övning, 1 varv. Detta innebär 5 totala intervaller. Sätt \`rounds\` till 5.
    *   **Struktur 5 (Stege):** 4 övningar, 20s arbete/övning, 3 varv. Detta innebär 12 totala intervaller (4 övn * 3 varv). Sätt \`rounds\` till 12.
*   **Konsistens:** Se till att värdet i \`settings.rounds\` exakt matchar det totala antalet arbetsintervaller för det upplägg du valt, och att antalet övningar i \`exercises\`-listan är korrekt för strukturen.
`;
...
export const WarmupScreen: React.FC<WarmupScreenProps> = ({ onStartWorkout }) => {
  ...
  const handleAddExercise = () => {
    if (warmupBlock) {
      ...
      handleUpdateBlock({
        ...
        settings: {
          ...warmupBlock.settings,
          rounds: warmupBlock.settings.rounds + 1,
        },
      });
    }
  };
  ...
  const handleRemoveExercise = (exId: string) => {
    if (warmupBlock) {
      const originalLength = warmupBlock.exercises.length;
      const updatedExercises = warmupBlock.exercises.filter(ex => ex.id !== exId);
      
      if (updatedExercises.length < originalLength) {
        handleUpdateBlock({
          ...warmupBlock,
          exercises: updatedExercises,
          settings: {
            ...warmupBlock.settings,
            rounds: Math.max(0, warmupBlock.settings.rounds - 1),
          },
        });
      }
    }
  ...
  const calculateTotalTime = (block: WorkoutBlock | null) => {
    ...
    switch (settings.mode) {
        ...
        case TimerMode.Tabata:
            ...
            const totalRests = totalWorkIntervals > 1 ? totalWorkIntervals - 1 : 0;
  ...
  const settingsText = useMemo(() => {
    ...
    const desc = `${settings.rounds} omgångar, ${exercises.length} övningar`;
    return `${time} • ${desc}`;
  }, [warmupBlock]);
