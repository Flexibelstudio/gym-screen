...
// HYROX-SPECIFIC HIGH DENSITY LIST
...
const CircuitView: React.FC<{ 
    ...
    return (
        <div className="w-full max-w-[1600px] grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 max-h-[45vh] overflow-y-auto pr-2 scrollbar-hide">
            {exercises.map((ex, index) => (
                <div key={ex.id} className="bg-white/90 dark:bg-gray-900/90 backdrop-blur-sm rounded-2xl p-6 shadow-lg flex items-center gap-5 border border-transparent">
                    <div className="w-16 h-16 rounded-full flex items-center justify-center text-white font-black text-3xl flex-shrink-0 shadow-lg" style={{ backgroundColor: `rgb(${timerStyle.pulseRgb})` }}>
                        {index + 1}
                    </div>
                    <div className="flex-grow min-w-0">
                        <h4 className="text-2xl md:text-3xl font-black text-gray-900 dark:text-white leading-tight truncate">{ex.name}</h4>
                        {ex.reps ? (
...
export const TimerScreen: React.FC<TimerScreenProps> = ({ 
  ...
  if (isHyroxRace) {
      ...
      return (
        <div 
            className={`fixed inset-0 w-full h-full overflow-hidden transition-colors duration-500 ${timerStyle.bg}`}
        ...
        >
          ...
          {/* CONTROLS */}
          <div className={`fixed z-50 transition-all duration-500 flex gap-6 left-[35%] -translate-x-1/2 bottom-8 ${controlsVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4 pointer-events-none'}`}>
                {status === TimerStatus.Idle || status === TimerStatus.Finished ? (
                    ...
                    <>
                        ...
                        <button onClick={() => setShowResetConfirmation(true)} className="bg-gray-700 text-white font-bold py-4 px-10 rounded-full shadow-xl hover:bg-gray-600 transition-colors text-xl border-2 border-gray-600">AVSLUTA</button>
                    </>
                ) : (
                ...
                {status !== TimerStatus.Running && <button onClick={() => setShowBackToPrepConfirmation(true)} className="bg-gray-800/80 text-white font-bold py-4 px-8 rounded-full shadow-xl border-2 border-gray-600 hover:bg-gray-700 transition-colors text-lg">⚙️ Grupper</button>}
          </div>
        </div>
      );
  }

  // 2. DEFAULT VIEW: ORIGINAL LAYOUT (UNTOUCHED)
  return (
    <div 
    ...
    >
      ...
      <div className={`absolute bottom-0 left-0 right-0 flex flex-col items-center justify-start px-4 z-0 ${showFullScreenColor ? 'top-[62%]' : 'top-[43%]'}`}>
      ...
      <div className={`fixed z-50 transition-all duration-500 flex gap-6 left-1/2 -translate-x-1/2 ${showFullScreenColor ? 'top-[62%]' : 'top-[46%]'} ${controlsVisible ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4 pointer-events-none'}`}>
            {status === TimerStatus.Idle || status === TimerStatus.Finished ? (
                <>
                    <button onClick={() => onFinish({ isNatural: false })} className="bg-gray-600/80 text-white font-bold py-4 px-10 rounded-full shadow-xl hover:bg-gray-500 transition-colors text-xl backdrop-blur-md border-2 border-white/20">TILLBAKA</button>
                    <button onClick={start} className="bg-white text-black font-black py-4 px-16 rounded-full shadow-2xl hover:scale-105 transition-transform text-xl border-4 border-white/50">STARTA</button>
                ...
                <>
                    <button onClick={resume} className="bg-green-500 text-white font-bold py-4 px-10 rounded-full shadow-xl hover:bg-green-400 transition-colors text-xl border-2 border-green-400">FORTSÄTT</button>
                    <button onClick={reset} className="bg-gray-700 text-white font-bold py-4 px-10 rounded-full shadow-xl hover:bg-gray-600 transition-colors text-xl border-2 border-gray-600">AVSLUTA</button>
                </>
            ) : (
                <button onClick={pause} className="bg-white text-gray-900 font-black py-4 px-16 rounded-full shadow-2xl hover:bg-gray-100 transition-transform hover:scale-105 text-xl border-4 border-white/50">PAUSA</button>
            )}
