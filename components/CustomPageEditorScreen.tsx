        ...
        <div className="w-full max-w-7xl mx-auto space-y-6 pb-28 animate-fade-in">
             <div className="bg-gray-800 p-6 rounded-lg space-y-6 border border-gray-700">
                <div className="flex justify-between items-center flex-wrap gap-4">
                    <h2 className="text-2xl font-bold text-white">{pageToEdit ? 'Redigera infosida' : 'Skapa ny infosida'}</h2>
                 ...
                 <div>
                    <label htmlFor="page-title" className="block text-sm font-medium text-gray-300 mb-1">Sidans Huvudtitel</label>
                    <input
                        id="page-title"
                        type="text"
                        value={title}
                        ...
                        className="w-full bg-black text-white p-3 rounded-md border border-gray-600 focus:ring-2 focus:ring-primary focus:outline-none text-xl"
                    />
                </div>
                   ...
                   {/* TAB CONTROLS */}
                           ...
                           {tabs.map((tab, index) => (
                               <div key={tab.id} className="relative">
                                    <button
                                      ...
                                      className={`px-4 py-3 font-semibold transition-colors focus:outline-none whitespace-nowrap rounded-t-md ${activeTabIndex === index ? 'bg-gray-700 text-primary' : 'text-gray-400 hover:bg-gray-700/50'}`}
                                    >
                            ...
                            <button onClick={handleAddTab} className="p-3 text-primary hover:bg-gray-700/50 rounded-md">+</button>
                        </div>
                    </div>
                    ...
                    <div className={`grid grid-cols-1 ${viewMode === 'preview' ? 'lg:grid-cols-2' : ''} gap-6`}>
                        ...
                        <div className="flex flex-col gap-4 min-h-[50vh]">
                            <h3 className="text-lg font-semibold text-white">Redigera Flik: <span className="text-primary">{activeTab?.title}</span></h3>
                            <div>
                                <label htmlFor="tab-title" className="block text-sm font-medium text-gray-400 mb-1">Flikens Titel</label>
                                <input
                                    id="tab-title"
                                    type="text"
                            ...
                            <div className="flex flex-col flex-grow">
                                <label htmlFor="page-content" className="block text-sm font-medium text-gray-400 mb-1">Innehåll (Stöder Markdown)</label>
                                <textarea
                                    id="page-content"
                        ...
                        {/* Right Pane: Preview */}
                         {viewMode === 'preview' && (
                            <div className="flex flex-col gap-2 min-h-[50vh]">
                                <label className="block text-lg font-semibold text-white flex-shrink-0">Förhandsgranskning</label>
                                <div className="bg-white dark:bg-black rounded-lg border border-gray-300 dark:border-gray-600 flex-grow">
