...
export const CoachScreen: React.FC<CoachScreenProps> = ({ role, navigateTo, onSelectCustomPage, isImpersonating, onReturnToAdmin }) => {
  ...
  if (selectedOrganization?.displayWindows && selectedOrganization.displayWindows.some(w => w.isEnabled)) {
    staticMenuItems.push({ title: 'Starta Skyltfönster', action: () => navigateTo(Page.DisplayWindowSelection) });
  }
