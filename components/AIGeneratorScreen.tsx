...
import { SparklesIcon, DocumentTextIcon, CloseIcon } from './icons';
    ...
    const handleAction = async () => {
        ...
        try {
            ...
            if (activeTab === 'generate') {
                ...
                if (selectedCategory) {
                    finalPrompt = `Skapa ett pass av typen "${selectedCategory.name}". ${selectedCategory.prompt}. ${prompt}`;
                } else if (!prompt.trim()) {
            ...
            // Deep clone and ensure all IDs/meta are present
        ...
        <div className="max-w-4xl mx-auto space-y-8 animate-fade-in pb-20">
            <div className="text-center space-y-4">
                ...
                <p className="text-gray-600 dark:text-gray-400 text-lg">
                    {activeTab === 'generate' 
                        ? 'Välj en passtyp och lägg till egna önskemål.' 
            ...
            <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-6 shadow-xl relative overflow-hidden transition-colors">
                ...
                {activeTab === 'generate' && studioConfig?.customCategories && (
                    <div className="mb-6">
                        <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-3">Välj passtyp:</label>
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                            {studioConfig.customCategories.map(cat => (
                                <button
                                    key={cat.id}
                                    onClick={() => setSelectedCategory(cat)}
                                    className={`py-3 px-4 rounded-xl font-bold text-sm transition-all shadow-sm ${selectedCategory?.id === cat.id ? 'bg-primary text-white ring-2 ring-primary ring-offset-2 ring-offset-white dark:ring-offset-gray-900' : 'bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700'}`}
                                >
                                    {cat.name}
                                </button>
                            ))}
                ...
                {activeTab === 'parse' && (
                    <div 
                        ...
                        className={`mb-6 w-full p-6 border-2 border-dashed rounded-xl transition-all cursor-pointer flex flex-col items-center justify-center min-h-[120px] relative ${isDragging ? 'border-primary bg-primary/10 scale-[1.01]' : 'border-gray-300 dark:border-gray-700 hover:border-gray-400 dark:hover:border-gray-600 bg-gray-50 dark:bg-gray-800/50'}`}
                    >
                        <input type="file" ref={fileInputRef} onChange={(e) => { if(e.target.files && e.target.files[0]) handleFile(e.target.files[0]) }} accept="image/*" className="hidden" />
                        {selectedImage ? (
                            <div className="relative w-full h-48 group" onClick={(e) => e.stopPropagation()}>
                                ...
                                <button onClick={() => { setSelectedImage(null); if(fileInputRef.current) fileInputRef.current.value = ''; }} className="absolute top-2 right-2 bg-red-600 text-white p-2 rounded-full shadow-lg hover:bg-red-700 transition-colors"><CloseIcon className="w-4 h-4" /></button>
                            </div>
                        ) : (
                            <div className="text-center pointer-events-none">
                                <div className="mx-auto w-12 h-12 bg-gray-200 dark:bg-gray-700 rounded-full flex items-center justify-center mb-3"><svg xmlns="http://www.w3.org/2000/svg" className="w-6 h-6 text-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg></div>
                                <p className="text-gray-700 dark:text-gray-300 font-bold text-sm">{isDragging ? 'Släpp bilden här!' : 'Klicka eller dra en bild hit'}</p>
                            </div>
                        )}
                    </div>
                )}
                ...
                <div className="space-y-2">
                    <label className="block text-sm font-bold text-gray-700 dark:text-gray-300">{activeTab === 'generate' ? 'Dina tillägg:' : (selectedImage ? 'Tillägg till bilden (valfritt):' : 'Klistra in texten här:')}</label>
                    <textarea
                        value={prompt}
                        onChange={(e) => setPrompt(e.target.value)}
                        placeholder={activeTab === 'generate' ? "Lägg till egna önskemål..." : "Klistra in ditt pass här..."}
                        className="w-full bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white p-4 rounded-xl border border-gray-200 dark:border-gray-700 focus:border-purple-500 focus:ring-1 focus:ring-purple-500 outline-none resize-none min-h-[120px] text-base transition-colors placeholder-gray-400"
                ...
                <div className="mt-6 flex justify-end">
                    <button
                        ...
                        className={`${activeTab === 'generate' ? 'bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-500 hover:to-pink-500' : 'bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500'} text-white font-bold py-3 px-8 rounded-xl shadow-lg transition-all transform active:scale-95 disabled:opacity-50 flex items-center gap-2 w-full sm:w-auto justify-center`}
                    >
                ...
                {error && <div className="mt-4 p-3 bg-red-100 dark:bg-red-900/30 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-200 rounded-lg text-sm text-center">{error}</div>}
            </div>
                ...
                <div className="animate-slide-up space-y-6">
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                        <h3 className="text-xl font-bold text-gray-900 dark:text-white flex items-center gap-3">
                            <span className="bg-blue-100 text-blue-700 dark:bg-blue-500/20 dark:text-blue-300 px-3 py-1 rounded-lg text-sm uppercase font-extrabold border border-blue-200">Förslag:</span> 
                            <span className="truncate max-w-[300px] sm:max-w-md">{generatedWorkout.title}</span>
                        </h3>
                        <div className="flex gap-4 w-full sm:w-auto">
                            <button onClick={() => setGeneratedWorkout(null)} className="text-gray-500 hover:text-red-500 dark:text-gray-400 px-4 py-3 font-medium transition-colors">Kasta</button>
                            <button onClick={handleAccept} className="flex-grow sm:flex-grow-0 bg-white dark:bg-white text-black hover:bg-gray-100 font-black py-3 px-8 rounded-xl shadow-xl transition-all transform hover:scale-105 flex items-center justify-center gap-2"><span>Använd detta pass</span><span className="text-lg">&rarr;</span></button>
                        </div>
                    </div>

                    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                        {generatedWorkout.blocks.map((block, i) => (
                            <div key={block.id || i} className="bg-white dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-xl p-5 shadow-sm">
                                ...
                                <ul className="space-y-2">
                                    {(block.exercises || []).map((ex, j) => (
                                        <li key={ex.id || j} className="text-sm text-gray-700 dark:text-gray-300 flex items-start gap-2">
                                            ...
                                            <span><strong className="text-gray-900 dark:text-white">{ex.name}</strong> {ex.reps && <span className="text-gray-500 mx-1">• {ex.reps}</span>}</span>
                                        </li>
                                    ))}
                                </ul>
                            </div>
