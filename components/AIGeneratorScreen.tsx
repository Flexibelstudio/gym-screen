...
const SubViewWrapper: React.FC<{ title: string; onBack: () => void; children: React.ReactNode }> = ({ title, onBack, children }) => (
    <div className="space-y-4 max-w-2xl mx-auto text-left">
        <button onClick={onBack} className="text-primary hover:underline font-semibold mb-2 text-sm">
...
export const AIGeneratorScreen: React.FC<AIGeneratorScreenProps> = ({ 
  ...
  const renderContent = () => {
    switch (mode) {
      case 'generate':
        return (
          <SubViewWrapper title="Skapa med AI" onBack={handleBackToHub}>
      ...
      case 'parse':
        return (
          <SubViewWrapper title="Klistra in & Tolka Pass" onBack={handleBackToHub}>
              <p className="text-gray-500 dark:text-gray-400 text-center pb-2">
      ...
      case 'manage':
        return (
          <div className="space-y-8 text-left">
                <button onClick={handleBackToHub} className="text-primary hover:underline font-semibold text-sm">
                  &larr; Tillbaka till val
                </button>
                <div>
