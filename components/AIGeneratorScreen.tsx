...
export const AIGeneratorScreen: React.FC<AIGeneratorScreenProps> = ({ 
  ...
  // Handle auto-expansion of a specific category (e.g. after saving a draft)
  useEffect(() => {
      if (initialExpandedCategory) {
          setExpandedCategories(prev => ({
              ...prev,
              [initialExpandedCategory]: true
          }));
      }
  }, [initialExpandedCategory]);
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (mode === 'parse') {
        ...
        generatedData = await generateWorkout(finalPrompt, workouts, selectedCategory);
  ...
  const renderContent = () => {
    if (isLoading) {
        return (
             <SubViewWrapper title="Skapar med AI...">
                <div className="text-center p-8 flex flex-col items-center justify-center gap-4">
                    ...
                    <p className="text-gray-500 dark:text-gray-400">
                        Vänta några sekunder medan coachen analyserar och laddar upp förslag direkt i byggaren.
                    </p>
                </div>
            </SubViewWrapper>
        );
              ...
              <p className="text-gray-500 dark:text-gray-400 text-center pb-2">
                {isFromBoost
                  ...
                  : (isGeneralMode
                    ...
                    : "AI:n kommer använda de förvalda instruktionerna. Lägg till egna önskemål för att anpassa passet.")
                }
              </p>
              {isGeneralMode && (
                <div>
