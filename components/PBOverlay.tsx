...
export const PBOverlay: React.FC<PBOverlayProps> = React.memo(({ isGrattisOpen }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = listenForStudioEvents(
      ...
      (event) => {
        ...
        if (shouldFilter) {
          if (event.locationId !== resolvedLocationId) {
  ...
  useEffect(() => {
    ...
    isGrattisReady,
    isGrattisOpen,
  ]); // Körs när vi får signal om nytt event, när ett event är klart, när timer-status ändras, eller när 5 sekunder har gått
