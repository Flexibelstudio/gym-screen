                     ...
                     <div className="bg-white dark:bg-gray-900 p-6 rounded-lg border border-slate-200 dark:border-gray-700 space-y-4">
                             ...
                             <div className="mt-6 pt-6 border-t border-gray-100 dark:border-gray-800">
                                 ...
                                 {organization.stripeConnectAccountId ? (
                                     ...
                                     <div className={`flex items-center justify-between p-4 rounded-lg border ${organization.allowStripeBypass ? 'bg-slate-50 dark:bg-slate-800/50 border-slate-200 dark:border-slate-700' : 'bg-orange-50 dark:bg-orange-900/20 border-orange-200 dark:border-orange-800'}`}>
                                         <div className="flex items-center gap-3">
                                             <div className={`w-8 h-8 rounded-full flex items-center justify-center ${organization.allowStripeBypass ? 'bg-slate-200 dark:bg-slate-700 text-slate-500' : 'bg-orange-100 dark:bg-orange-800 text-orange-600 dark:text-orange-400'}`}>
                                                 {organization.allowStripeBypass ? (
                                                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                                 ) : (
                                                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path></svg>
                                                 )}
                                             ...
                                             <div>
                                                 <p className={`font-bold ${organization.allowStripeBypass ? 'text-slate-700 dark:text-slate-300' : 'text-orange-800 dark:text-orange-300'}`}>
                                                     {organization.allowStripeBypass ? 'Bypass för Stripe aktiverat' : 'Konto ej kopplat'}
                                                 </p>
                                                 <p className={`text-xs ${organization.allowStripeBypass ? 'text-slate-500' : 'text-orange-600 dark:text-orange-400'}`}>
                                                     {organization.allowStripeBypass ? 'Betalningar hanteras via systemägare' : 'Krävs för att ta emot betalningar'}
                                                 </p>
                                             </div>
                                         </div>
                                         <div className="flex items-center gap-2">
                                             <button 
                                                ...
                                                disabled={isConnectingStripe}
                                                className={`text-sm px-4 py-2 rounded-lg transition-colors shadow-sm disabled:opacity-50 flex items-center justify-center gap-2 ${organization.allowStripeBypass ? 'bg-white dark:bg-gray-800 border border-slate-300 dark:border-gray-600 text-slate-700 dark:text-gray-300 hover:bg-slate-50 dark:hover:bg-gray-700' : 'bg-primary text-white hover:bg-primary/90'}`}
                                             >
                                                 {isConnectingStripe ? (
                                                     <>
                                                         <svg className={`animate-spin h-4 w-4 ${organization.allowStripeBypass ? 'text-slate-500' : 'text-white'}`} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                                             <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                                                             <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                                                         </svg>
                                                         Laddar...
                                                     </>
                                                 ) : (organization.allowStripeBypass ? 'Koppla konto ändå' : 'Koppla konto nu')}
                                             </button>
                                         </div>
                                     </div>
                                 )}
