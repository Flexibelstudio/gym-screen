...
export const HomeScreen: React.FC<HomeScreenProps> = ({ 
  ...
  const menuItems = useMemo(() => {
    ...
    items.push({ title: 'Sparade pass', action: () => navigateTo(Page.SavedWorkouts) });
    items.push({ title: 'För coacher', action: onCoachAccessRequest });
    
    return items;
  }, [studioConfig, navigateTo, onSelectPasskategori, onShowBoostModal, onCreateNewWorkout, onCoachAccessRequest]);
    ...
    <div className="w-full max-w-5xl mx-auto text-center">
      {organizationLogoUrl && (
          <div className="mb-12 flex justify-center">
              <img src={organizationLogoUrl} alt="Organisationslogotyp" className="max-h-40 max-w-md object-contain" />
          </div>
      )}
