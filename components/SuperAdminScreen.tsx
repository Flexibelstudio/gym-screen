...
import { StudioConfig, Studio, Organization, CustomPage, CustomCategoryWithPrompt, Page, UserData, UserRole, EquipmentItem, InfoCarousel, InfoMessage, DisplayWindow, DisplayPost, Workout } from '../types';
import { ToggleSwitch, HomeIcon, DocumentTextIcon, SpeakerphoneIcon, SparklesIcon, UsersIcon } from './icons';
...
type AdminTab = 
    ...
    'globala-installningar' | 'studios' | 'varumarke' |
    'anvandare';
...
// FIX: Define the missing AnvandareContent component.
const AnvandareContent: React.FC<SuperAdminScreenProps & { admins: UserData[], coaches: UserData[], isLoading: boolean }> = ({ organization, admins, coaches, isLoading }) => {
    ...
    const handleInviteUser = async (e: React.FormEvent) => {
        ...
        try {
            const result = await inviteUserCall({
                email: inviteEmail,
                role: inviteRole,
            ...
            if (result.success) {
                ...
                setInviteEmail('');
                setInvitePassword('');
                // A production app would likely trigger a re-fetch of users here.
            }
        } catch (error) {
            setInviteError(error instanceof Error ? error.message : "Ett okänt fel inträffade.");
        } finally {
            setIsInviting(false);
        }
    };
    ...
    return (
        <div className="space-y-8">
            <div className="bg-slate-100 dark:bg-gray-800 p-6 rounded-lg space-y-4 border border-slate-200 dark:border-gray-700">
                <h3 className="text-2xl font-bold text-gray-900 dark:text-white border-b border-slate-300 dark:border-gray-700 pb-3 mb-4">Bjud in Användare</h3>
                <form onSubmit={handleInviteUser} className="space-y-4">
                    <input type="email" value={inviteEmail} onChange={e => setInviteEmail(e.target.value)} placeholder="E-postadress" required className="w-full bg-white dark:bg-black p-3 rounded-md border border-slate-300 dark:border-gray-600" />
                    <input type="password" value={invitePassword} onChange={e => setInvitePassword(e.target.value)} placeholder="Temporärt lösenord (min 6 tecken)" required className="w-full bg-white dark:bg-black p-3 rounded-md border border-slate-300 dark:border-gray-600" />
                    <select value={inviteRole} onChange={e => setInviteRole(e.target.value as 'admin' | 'coach')} className="w-full bg-white dark:bg-black p-3 rounded-md border border-slate-300 dark:border-gray-600">
                        <option value="coach">Coach</option>
                        <option value="admin">Organisationsadmin</option>
                    </select>
                    <button type="submit" disabled={isInviting} className="w-full bg-primary hover:brightness-95 text-white font-bold py-3 rounded-lg disabled:opacity-50">
                        {isInviting ? 'Bjuder in...' : 'Skicka Inbjudan'}
                    </button>
                    {inviteSuccess && <p className="text-green-400 text-center">{inviteSuccess}</p>}
                    {inviteError && <p className="text-red-400 text-center">{inviteError}</p>}
                </form>
            </div>
...
export const SuperAdminScreen: React.FC<SuperAdminScreenProps> = (props) => {
    const { organization, theme, onSaveGlobalConfig } = props;
...
// FIX: Define the missing CategoryPromptManager component.
...
// FIX: Define the missing GlobalEquipmentManager component.
...
const GlobalEquipmentManager: React.FC<GlobalEquipmentManagerProps> = ({ equipment, onEquipmentChange, isSaving }) => {
    const handleUpdateItem = (id: string, name: string, quantity: number) => {
        onEquipmentChange(equipment.map(item => item.id === id ? { ...item, name, quantity } : item));
    };
    ...
    return (
        <div className="space-y-4">
            <p className="text-sm text-gray-600 dark:text-gray-400">
                Detta är den globala listan på utrustning som är tillgänglig för alla studios. Studios kan sedan anpassa antal eller lägga till egna unika redskap.
            </p>
            {equipment.map(item => (
                <div key={item.id} className="bg-slate-200/50 dark:bg-black/50 p-4 rounded-lg border border-slate-300 dark:border-gray-600 flex justify-between items-center gap-4">
                    <input 
                        type="text" 
                        value={item.name} 
                    ...
                    <input 
                        type="number" 
                        value={item.quantity} 
                        onChange={(e) => handleUpdateItem(item.id, item.name, parseInt(e.target.value, 10) || 0)} 
            ...
            <button onClick={handleAddItem} disabled={isSaving} className="w-full mt-4 bg-primary/20 hover:bg-primary/40 text-primary font-bold py-2 px-4 rounded-lg transition-colors border-2 border-dashed border-primary/50">
...
const GlobalaInställningarContent: React.FC<SuperAdminScreenProps & ConfigProps> = ({ config, isSavingConfig, isConfigDirty, handleUpdateConfigField, handleSaveConfig, organization }) => {
    ...
    // FIX: Removed unused onAutoSave handler. Component changes will be saved with the main button.
    ...
    return (
        <div className="bg-slate-100 dark:bg-gray-800 p-6 rounded-lg space-y-6 border border-slate-200 dark:border-gray-700">
            ...
            <div className="space-y-4">
                ...
                <ToggleSwitch label="Aktivera Skärmsläckare (15 min)" checked={config.enableScreensaver ?? false} onChange={(c) => handleUpdateConfigField('enableScreensaver', c)} />
            </div>
            ...
            <div className="space-y-2 pt-4 border-t border-slate-300 dark:border-gray-700">
                 <h4 className="font-semibold text-gray-700 dark:text-gray-300">Global Utrustningslista</h4>
...
const DisplayPostEditor: React.FC<DisplayPostEditorProps> = ({ post: initialWindow, studios, onSave, onCancel, isSaving = false }) => {
...
const DisplayPostForm: React.FC<DisplayPostFormProps> = ({ post, studios, onSave, onCancel, isSaving = false }) => {
