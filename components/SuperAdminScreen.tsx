...
import { StudioConfig, Studio, Organization, CustomPage, CustomCategoryWithPrompt, Page, UserData, UserRole } from '../types';
...
type AdminTab = 'drift' | 'organisation' | 'admin';
    ...
    return (
        <div className="w-full max-w-4xl mx-auto space-y-8 animate-fade-in pb-12">
            ...
            <div className="space-y-8">
                {activeTab === 'drift' && <DriftContent {...props} />}
...
const DriftContent: React.FC<SuperAdminScreenProps> = ({ organization, onSaveGlobalConfig, onEditStudioConfig, onCreateStudio, onUpdateStudio, onDeleteStudio, onPassProgramNavigation, onUpdateCustomPages, onSwitchToStudioView }) => {
        ...
        <>
             <div className="bg-gray-800 p-6 rounded-lg space-y-4 border border-gray-700">
                ...
                <div className="pt-4 flex justify-start items-center">
                    <button onClick={() => handleOpenEditor(null)} className="bg-primary hover:brightness-95 text-white font-bold py-2 px-5 rounded-lg">
                        Lägg till ny sida
                    </button>
                </div>
            </div>
            ...
            <div className="bg-gray-800 p-6 rounded-lg space-y-6 border border-gray-700">
                ...
                <div className="space-y-4">
                    ...
                    {config.enableBoost && (
                        <div className="pl-8 border-l-2 border-gray-700 ml-3 py-2">
                            <ToggleSwitch
                                label="Aktivera QR-kod för loggning"
                                checked={!!config.enableBoostQrLogging}
                                onChange={(c) => setConfig(prev => ({...prev, enableBoostQrLogging: c}))}
                            />
                            <p className="text-xs text-gray-500 mt-1 pl-2">Visar QR-kodsknappen på detaljsidan för Dagens Boost-pass.</p>
                        </div>
                    )}
...
const CategoryPromptManager: React.FC<CategoryPromptManagerProps> = ({ categories, onCategoriesChange, isSaving }) => {
    ...
    const handleUpdateCategory = (id: string, field: 'name' | 'prompt' | 'enableQrLogging', value: string | boolean) => {
        ...
        <div className="space-y-4">
            ...
            {categories.map(cat => (
                <div key={cat.id} className="bg-black/50 p-4 rounded-lg border border-gray-600 space-y-3">
                    ...
                    <ToggleSwitch 
                        label="Aktivera QR-kod för loggning"
                        checked={!!cat.enableQrLogging}
                        onChange={(checked) => handleUpdateCategory(cat.id, 'enableQrLogging', checked)}
                    />
                </div>
            ))}
