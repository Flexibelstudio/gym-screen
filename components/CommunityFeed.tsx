...
export const CommunityFeed: React.FC<CommunityFeedProps> = ({ onExpand, isExpanded = false }) => {
    ...
    return (
        ...
        >
            ...
            <div 
                className={`flex-grow overflow-y-auto pr-1 relative z-10 custom-scrollbar scroll-smooth`}
                        ...
                        {logs.length > 0 ? (
                            logs.map((log) => (
                                <motion.div
                                    ...
                                    className="bg-white/40 dark:bg-black/30 hover:bg-white/50 dark:hover:bg-black/40 transition-colors rounded-2xl flex items-center gap-4 border border-gray-100 dark:border-white/5 group px-4 overflow-hidden"
                                >
