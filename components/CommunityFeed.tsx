...
// Helper to format relative time (e.g. "5 min sedan")
...
export const CommunityFeed: React.FC = () => {
    ...
    useEffect(() => {
        ...
        setIsLoading(true);
        // Listen to the last 20 logs in real-time
    ...
    // Force update relative times every minute
    ...
    if (isLoading) {
        return (
            <div className="h-full flex items-center justify-center text-white/50 text-sm italic">
                Laddar flödet...
            </div>
        );
    }
    ...
    if (logs.length === 0) {
        return (
            <div className="bg-white/10 backdrop-blur-md rounded-3xl p-6 border border-white/10 h-full flex flex-col items-center justify-center text-center">
    ...
    return (
        <div className="bg-gradient-to-b from-white/10 to-white/5 backdrop-blur-md rounded-3xl p-6 border border-white/10 h-full flex flex-col relative overflow-hidden shadow-2xl">
            ...
            {/* Feed List */}
            <div className="flex-grow overflow-y-auto pr-2 space-y-3 scrollbar-hide relative z-10 mask-image-b-fade">
                <AnimatePresence initial={false}>
                    {logs.map((log, index) => (
                        <motion.div
                            ...
                            className="bg-black/40 hover:bg-black/60 transition-colors rounded-xl p-3 flex items-center gap-4 border border-white/5 group"
                        >
                            {/* Avatar */}
                            ...
                            <div className="flex-grow min-w-0">
                                ...
                                <div className="flex items-center justify-between">
                                    <p className="text-white/70 text-xs truncate max-w-[80%] group-hover:text-primary transition-colors">
                                        {log.workoutTitle}
                                    </p>
                                    {log.feeling && (
                                        <span className="text-sm transform group-hover:scale-125 transition-transform" title="Känsla">
                                            {getFeelingIcon(log.feeling)}
                                        </span>
                                    )}
                                </div>
                            </div>
            ...
            {/* Simple fade mask at bottom handled via CSS or gradient overlay */}
