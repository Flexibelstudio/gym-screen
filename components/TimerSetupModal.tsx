...
export const TimerSetupModal: React.FC<TimerSetupModalProps> = ({ isOpen, onClose, block, onSave }) => {
  ...
  const [rounds, setRounds] = useState(block.settings.rounds);
  ...
  useEffect(() => {
    if (block) {
        ...
        setRounds(block.settings.rounds);
  ...
  const handleSave = () => {
    ...
    switch(mode) {
      ...
      case TimerMode.EMOM:
        newSettings.rounds = rounds;
        newSettings.workTime = block.settings.workTime || 60; // Keep original or default to 60s
        break;
