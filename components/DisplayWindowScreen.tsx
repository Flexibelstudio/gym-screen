...
export const DisplayWindowScreen: React.FC<DisplayWindowScreenProps> = ({ onBack }) => {
    ...
    const displayConfig = selectedOrganization?.displayConfig;
    
    const activePosts = useMemo(() => {
        if (!displayConfig?.isEnabled || !selectedStudio || !displayConfig.posts) {
            return [];
        }
        const now = new Date();
        return displayConfig.posts.filter(post => {
    ...
    }, [displayConfig, selectedStudio]);
    
    const timerIdRef = useRef<number | null>(null);
    const advanceCallbackRef = useRef<(() => void) | undefined>();

    // Keep the ref pointing to the latest advance logic, now with transitions
    useEffect(() => {
        // FIX: Corrected the function signature for the callback ref.
        // It's only ever called without arguments, so `() => void` is the correct type.
    ...
    if (!displayConfig || !displayConfig.isEnabled) {
        return (
    ...
    return (
        <div className="fixed inset-0 bg-black text-white font-sans overflow-hidden">
            ...
            <button
                onClick={onBack}
                className="absolute top-4 right-4 bg-black/50 text-white/70 hover:bg-black hover:text-white rounded-full px-4 py-2 text-sm font-semibold transition-colors z-10"
            >
                Stäng
            </button>
        </div>
