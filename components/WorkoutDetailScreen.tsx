...
import { Workout, WorkoutBlock, TimerMode, TimerSettings, Exercise, WorkoutCategory } from '../types';
...
const WorkoutDetailScreen: React.FC<WorkoutDetailScreenProps> = ({ workout, onStartBlock, onUpdateBlockSettings, onEditWorkout, isCoachView, onTogglePublish, onDuplicate, onShowImage }) => {
  ...
  // When the source workout prop changes (e.g. navigating to a new workout), reset our session state
  useEffect(() => {
    setSessionWorkout(JSON.parse(JSON.stringify(workout)));
  }, [workout]);

  // Handle TEMPORARY updates to a block's exercises
  ...
  // Handle PERMANENT updates to a block's settings from the modal
  const handleUpdateSettings = (blockId: string, newSettings: Partial<TimerSettings>) => {
      // This prop call saves the settings permanently
      onUpdateBlockSettings(blockId, newSettings);

      // We also update our temporary session state to reflect the change immediately
      ...
      <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-6 shadow-md border border-gray-200 dark:border-gray-700">
        ...
        <div className="border-t border-gray-200 dark:border-gray-700 pt-4">
          ...
          {exercisesVisible && !isEditingExercises && (
            <div className={`grid ${block.exercises.length > 8 ? 'sm:grid-cols-2' : 'grid-cols-1'} gap-x-8 gap-y-1 pt-2`}>
