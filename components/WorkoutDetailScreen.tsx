...
export const WorkoutPresentationModal: React.FC<{ workout: Workout; onClose: () => void }> = ({ workout, onClose }) => {
    ...
    return (
        ...
        >
            <div className="flex justify-between items-center p-4 sm:p-6 lg:p-8 border-b border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-900/50 flex-shrink-0">
                <div className="flex items-center gap-3 sm:gap-4 lg:gap-6 flex-wrap min-w-0">
                    <h1 className="text-2xl sm:text-3xl lg:text-5xl font-black text-gray-900 dark:text-white uppercase tracking-tight leading-tight break-words">
                        {workout.title}
                    </h1>
                    <span className="text-[10px] sm:text-xs lg:text-sm font-bold bg-gray-200 dark:bg-gray-800 px-2.5 py-1 lg:px-4 lg:py-2 rounded-md text-gray-500 uppercase tracking-widest whitespace-nowrap">
                        Hela Passet
                    </span>
                </div>
                <button 
                    onClick={onClose}
                    className="p-2.5 sm:p-3 lg:p-4 bg-gray-200 dark:bg-gray-800 rounded-full hover:bg-gray-300 dark:hover:bg-gray-700 transition-colors shadow-sm active:scale-95 flex-shrink-0 ml-4"
                >
                    <CloseIcon className="w-6 h-6 lg:w-8 lg:h-8 text-gray-900 dark:text-white" />
                </button>
            </div>

            <div ref={scrollRef} className="flex-grow overflow-y-auto p-4 sm:p-8 lg:p-12 space-y-8 sm:space-y-12 lg:space-y-16">
                <div className="max-w-4xl lg:max-w-6xl mx-auto space-y-8 sm:space-y-12 lg:space-y-16">
                    {workout.blocks?.map((block, bIndex) => {
                        if (!block) return null;
                        return (
                        <div key={block.id || `block-${bIndex}`} className="space-y-4 sm:space-y-5 lg:space-y-8">
                            <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4 lg:gap-6 border-b-2 border-gray-100 dark:border-gray-800 pb-3 lg:pb-6">
                                <div className="flex items-center gap-3 lg:gap-5 flex-wrap">
                                    <span className={`inline-flex items-center px-3 py-1 lg:px-4 lg:py-2 rounded-lg text-xs sm:text-sm lg:text-base font-black uppercase tracking-[0.1em] shadow-sm ${getTagColor(block.tag)}`}>
                                        {block.tag}
                                    </span>
                                    <h2 className="text-xl sm:text-2xl lg:text-4xl font-black text-gray-900 dark:text-white uppercase tracking-tight break-words">
                                        {block.title}
                                    </h2>
                                </div>
                                <div className="sm:ml-auto flex flex-row sm:flex-col items-center sm:items-end gap-2 sm:gap-0 lg:gap-1">
                                    <span className="text-sm sm:text-base lg:text-xl font-mono font-bold text-gray-400">
                                        {block.settings.mode}
                                    </span>
                                    <span className="text-[10px] sm:text-xs lg:text-sm font-bold text-gray-500 uppercase tracking-widest break-words">
                            ...
                            {block.setupDescription && (
                                <p className="text-sm sm:text-base lg:text-xl text-gray-500 dark:text-gray-400 font-medium leading-relaxed max-w-3xl lg:max-w-4xl break-words">
                                    {block.setupDescription}
                                </p>
                            )}
                            ...
                            <div className="flex flex-col">
                                {block.exercises?.map((ex, index) => {
                                    ...
                                    const marginClass = isGroupedWithNext ? 'mb-1' : 'mb-3 lg:mb-4';

                                    return (
                                    <div key={ex.id || `ex-${index}`} className={`flex items-start gap-4 lg:gap-6 p-4 sm:p-5 lg:p-8 ${roundedClass} bg-gray-50 dark:bg-gray-900 ${borderClass} ${marginClass}`}>
                                         <div className="flex-shrink-0 w-8 h-8 sm:w-10 sm:h-10 lg:w-14 lg:h-14 rounded-full bg-gray-200 dark:bg-gray-800 flex items-center justify-center text-sm sm:text-base lg:text-xl font-black text-gray-500">
                                            {index + 1}
                                        </div>
                                        <div className="flex-grow min-w-0">
                                            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2 sm:gap-4 lg:gap-6">
                                                <h3 className="text-lg sm:text-xl lg:text-3xl font-black text-gray-900 dark:text-white leading-tight break-words">
                                                    {ex.name || 'Okänd övning'}
                                                </h3>
                                                {ex.reps && (
                                                    <div className="bg-primary/10 text-primary px-3 py-1.5 lg:px-5 lg:py-2.5 rounded-lg whitespace-nowrap self-start sm:self-auto">
                                                        <span className="text-sm sm:text-base lg:text-2xl font-mono font-black">{formatReps(ex.reps)}</span>
                                                    </div>
                                                )}
                                            </div>
                                            {ex.description && (
                                                <p className="text-sm lg:text-lg text-gray-500 dark:text-gray-400 mt-1.5 lg:mt-3 leading-relaxed font-medium break-words">
                                                    {ex.description}
                                                </p>
                                            )}
                                        </div>
                                ...
                                {block.exercises?.length === 0 && (
                                    <p className="text-sm lg:text-lg text-gray-400 italic pl-4 lg:pl-6">Inga övningar.</p>
                                )}
                            </div>
                        </div>
                    )})}
            ...
            <div className="p-6 lg:p-8 bg-gray-50 dark:bg-gray-900 border-t border-gray-100 dark:border-gray-800 flex justify-center flex-shrink-0">
                 <button onClick={onClose} className="bg-black dark:bg-white text-white dark:text-black font-black text-xl lg:text-2xl py-4 px-12 lg:py-5 lg:px-16 rounded-full shadow-xl hover:scale-105 transition-transform uppercase tracking-widest">
                     Stäng visningsläge
                 </button>
            </div>
        </motion.div>
