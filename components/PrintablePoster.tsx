import React, { useState, useEffect } from 'react';
import { createPortal } from 'react-dom';
import QRCode from 'qrcode';

interface PrintablePosterProps {
  title: string;
  code: string;
  url: string;
  organizationName: string;
}

export const PrintablePoster: React.FC<PrintablePosterProps> = ({ title, code, url, organizationName }) => {
  const [qrCodeDataUrl, setQrCodeDataUrl] = useState<string>('');

  useEffect(() => {
    if (url) {
      QRCode.toDataURL(url, { width: 880, margin: 1, errorCorrectionLevel: 'H' })
        .then(setQrCodeDataUrl)
        .catch(console.error);
    }
  }, [url]);

  const posterContent = (
    <div className="print-only w-full bg-white text-black flex flex-col items-center justify-start p-8" style={{ minHeight: '100vh', boxSizing: 'border-box', pageBreakInside: 'avoid' }}>
      <div className="max-w-3xl mx-auto flex flex-col items-center w-full pt-6 pb-4">
        <h2 className="text-xl font-bold text-gray-500 mb-2 uppercase tracking-widest text-center">{organizationName}</h2>
        <h1 className="text-4xl sm:text-5xl font-black mb-8 leading-tight uppercase max-w-2xl text-center text-black">{title}</h1>
        
        <div className="bg-gray-50 p-8 rounded-[3rem] border-4 border-gray-100 flex flex-col items-center mb-10 shadow-xl w-full max-w-sm mx-auto">
          {qrCodeDataUrl ? (
            <img src={qrCodeDataUrl} alt="QR Code" className="w-[220px] h-[220px] mb-6 rounded-xl bg-white" />
          ) : (
            <div className="w-[220px] h-[220px] mb-6 rounded-xl bg-gray-200 animate-pulse" />
          )}
          <p className="text-xs font-bold text-gray-400 uppercase tracking-[0.3em] mb-2 text-center">Din Inbjudningskod</p>
          <p className="text-6xl font-black font-mono tracking-[0.2em] text-center text-black">{code}</p>
        </div>

        <div className="text-left w-full max-w-xl space-y-5 mx-auto">
          <h3 className="text-2xl font-black uppercase tracking-widest border-b-2 border-gray-200 pb-3 mb-4 text-black">Så här gör du:</h3>
          <div className="flex items-start gap-5">
            <div className="w-10 h-10 rounded-full bg-black text-white flex items-center justify-center font-black text-xl flex-shrink-0 mt-1">1</div>
            <p className="text-xl font-semibold leading-relaxed text-black">Scanna QR-koden ovan med din mobilkamera, eller gå in på <span className="font-black">app.smartstudio.se</span> och ange koden <span className="font-black bg-gray-100 px-3 py-1 rounded-lg">{code}</span>.</p>
          </div>
          <div className="flex items-start gap-5">
            <div className="w-10 h-10 rounded-full bg-black text-white flex items-center justify-center font-black text-xl flex-shrink-0 mt-1">2</div>
            <p className="text-xl font-semibold leading-relaxed text-black">Klart! Lägg gärna till webbsidan på din hemskärm så att du lätt hittar tillbaka.</p>
          </div>
        </div>
      </div>
    </div>
  );

  // Render the poster directly into the body to avoid any layout interference from parent containers
  return createPortal(posterContent, document.body);
};
