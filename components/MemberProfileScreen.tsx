...
import { ChartBarIcon, DumbbellIcon, PencilIcon, SparklesIcon, UserIcon, FireIcon, LightningIcon, TrashIcon, CloseIcon, TrophyIcon, HistoryIcon, FlagIcon, StarIcon, ChevronRightIcon, SunIcon } from './icons';
...
import { calculateWeeklyStreak, getGoalCoachingAdvice, getMilestoneInfo, getAthleteArchetype } from './member-profile/profileHelpers';
import { LogDetailModal } from './member-profile/LogDetailModal';
import { PushNotificationSettings } from './member-profile/PushNotificationSettings';
import { BenchmarksView } from './member-profile/BenchmarksView';
import { SmartItem, ResumeWorkoutBanner, MonthlyWrappedBanner } from './member-profile/profileWidgets';
import { SummerChallengeDiplomaCard } from './member-profile/SummerChallengeDiplomaCard';
import { GoalsEditModal } from './member-profile/GoalsEditModal';
...
export const MemberProfileScreen: React.FC<MemberProfileScreenProps> = ({ userData, onBack, profileEditTrigger, navigateTo, functions, studioConfig }) => {
    ...
    const summerStats = useMemo(() => {
        ...
        if (completedPercentage >= 110) {
            ...
            label = 'HET';
            emoji = '🔥';
            colorClass = 'text-orange-500 dark:text-orange-400 font-extrabold';
    ...
    const handleUpdateLog = async (logId: string, updates: Partial<WorkoutLog>) => {
        ...
        if (updates.exerciseResults && userData.uid) {
            const exerciseNames = updates.exerciseResults.map(e => e.exerciseName).filter(Boolean);
            if (exerciseNames.length > 0) {
                try {
                    await recalculatePersonalBestsForExercises(userData.uid, exerciseNames);
                } catch (e) {
                    console.warn('PB recalculation after log update failed silently:', e);
                }
            }
        }
    };
    ...
    }, [logs, userData, isUserJoined, configToUse]);
    ...
    return (
        <div className="w-full max-w-4xl mx-auto px-0.5 sm:px-3 pt-2 pb-24 animate-fade-in relative z-0 overflow-x-clip">
            ...
            {activeTab === 'overview' && (
                <div className="space-y-3 sm:space-y-5 animate-fade-in">
                    ...
                    <WeeklyGoalRing 
                        ...
                        hasSummerSisu={isSummerThemeActive && isUserJoined && isChallengeStarted && !isChallengeEnded}
                    ...
                    {isSummerThemeActive && isUserJoined && !isChallengeFullyExpired && (
                        isChallengeEnded ? (
                            <>
                                <div className="relative overflow-hidden bg-white dark:bg-gray-800 text-gray-900 dark:text-white border border-gray-200 dark:border-gray-700 rounded-[2rem] p-6 sm:p-8 shadow-sm text-left space-y-5 animate-fade-in">
                                    {/* Small sunny decorative background */}
                                    <div className="absolute top-[-30px] right-[-30px] w-48 h-48 bg-amber-500/10 rounded-full blur-3xl pointer-events-none"></div>

                                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 relative z-10 w-full">
                                        <div className="flex items-center gap-3">
                                            <div className="w-12 h-12 bg-amber-100 dark:bg-amber-950/40 rounded-2xl flex items-center justify-center text-2xl select-none">
                                                ☀️
                                            </div>
                                            <div>
                                                <span className="text-[10px] font-black uppercase tracking-widest text-amber-600 dark:text-amber-400 block mb-0.5">Slutresultat</span>
                                                <h4 className="text-xl font-black text-gray-900 dark:text-white leading-none">
                                                    {configToUse?.summerChallengeTitle || "Sommarutmaningen ☀️"}
                                                </h4>
                                            </div>
                                        </div>
                                        
                                        <span className="inline-flex py-1 px-3 rounded-full bg-amber-100 dark:bg-amber-950/40 text-amber-800 dark:text-amber-300 font-extrabold text-xs items-center gap-1.5 self-start sm:self-auto select-none">
                                            🏆 Genomförd
                                        </span>
                                    </div>

                                    <p className="text-sm font-medium text-gray-600 dark:text-gray-300 leading-relaxed max-w-xl">
                                        Stort tack för ditt fantastiska deltagande! Med oerhörd glöd har vårt gym tillsammans nått temperaturnivån <strong className="text-amber-600 dark:text-amber-400">{summerStats.label} {summerStats.emoji}</strong> med totalt <strong>{grandTotalPointsForStudio} p</strong>. 
                                    ...
                                    <div className="flex flex-col sm:flex-row gap-3 pt-1 w-full">
                                        <button
                                            onClick={() => setIsSummerDiplomaOpen(true)}
                                            className="flex-1 bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 active:scale-95 text-white font-black uppercase text-[11px] tracking-wider py-3 px-5 rounded-2xl shadow-md hover:shadow-lg transition-all cursor-pointer flex items-center justify-center gap-2"
                                        >
                                            <span>📜</span> Visa mitt hedersdiplom
                                        </button>
                                        <button
                                                ...
                                                return (
                                                    <div
                                                        ...
                                                        className={`flex items-center justify-between p-2.5 rounded-xl text-xs font-semibold ${
                                                            isMe 
                                                            ? 'bg-amber-100 dark:bg-amber-950/40 text-amber-900 dark:text-amber-200 border border-amber-300/30 font-bold' 
                                                            : 'bg-gray-50 dark:bg-black/10 border border-transparent text-gray-700 dark:text-gray-300'
                                                        }`}
                                                    >
                                                        <div className="flex items-center gap-3">
                                    ...
                                    >
                                        <div className="p-5 sm:p-6 space-y-6 text-amber-950">
                                            ...
                                            {!isChallengeStarted ? (
                                                ...
                                                /* Beräkna relevanta timestamps för personliga mål */
                                                (() => {
                                                    const now = new Date();
                                                    const currentDay = now.getDay() || 7;
                                                    const mondaySec = new Date(now);
                                                    mondaySec.setDate(now.getDate() - (currentDay - 1));
                                                    mondaySec.setHours(0, 0, 0, 0);
                                                    const thisWeekMon = mondaySec.getTime();

                                                    const nextMondaySec = new Date(mondaySec);
                                                    nextMondaySec.setDate(mondaySec.getDate() + 7);
                                                    const nextWeekMon = nextMondaySec.getTime();

                                                    const baseGoalThisWeek = userData.summerChallengeGoals?.[thisWeekMon] !== undefined 
                                                         ? userData.summerChallengeGoals[thisWeekMon] 
                                                         : (userData.summerChallengeGoal || 3);

                                                     let myGoalThisWeek = baseGoalThisWeek;
                                                     const joinedAt = userData.joinedSummerChallengeAt || 0;
                                                     let isScaledWeek = false;
                                                     let originalGoal = baseGoalThisWeek;
                                                     let daysLeftInStartWeek = 0;
                                                     let joinDayName = 'onsdag';

                                                     if (joinedAt >= thisWeekMon && joinedAt < thisWeekMon + 7 * 24 * 60 * 60 * 1000) {
                                                         const joinDate = new Date(joinedAt);
                                                         const joinDay = joinDate.getDay() || 7; // Monday = 1, ..., Sunday = 7
                                                         daysLeftInStartWeek = Math.max(0, 7 - joinDay);
                                                         myGoalThisWeek = Math.max(1, Math.round((daysLeftInStartWeek / 7) * baseGoalThisWeek));
                                                         isScaledWeek = true;
                                                         
                                                         const swedishDays = ['söndag', 'måndag', 'tisdag', 'onsdag', 'torsdag', 'fredag', 'lördag'];
                                                         joinDayName = swedishDays[joinDate.getDay()];
                                                     }

                                                    const myGoalNextWeek = userData.summerChallengeGoals?.[nextWeekMon] !== undefined 
                                                        ? userData.summerChallengeGoals[nextWeekMon] 
                                                        : (userData.nextSummerChallengeGoal || userData.summerChallengeGoal || 3);

                                                    const personalPct = Math.round((stats.summerWeekPoints / myGoalThisWeek) * 100);
                                                    let personalFeedback = 'Kom igen! Varje pass räknas ❄️';
                                                    if (personalPct >= 110) {
                                                        personalFeedback = 'Överhettning! Du är magisk! 🌋';
                                                    } else if (personalPct >= 100) {
                                                        personalFeedback = 'Målet nått! Snyggt jobbat 🔥';
                                                    } else if (personalPct >= 70) {
                                                        personalFeedback = 'Snart där, du är grym! ☀️';
                                                    } else if (personalPct >= 40) {
                                                        personalFeedback = 'Bra början, fortsätt så! 🌤️';
                                                    }
                                                            ...
                                                            {/* Mina Personliga Framsteg block */}
                                                             <div className="bg-amber-50/70 dark:bg-amber-950/20 backdrop-blur-md rounded-2xl p-4 sm:p-5 border border-amber-200/35 text-amber-950">
                                                                 ...
                                                                 <div className="flex justify-between items-center mb-3">
                                                                    ...
                                                                    <span className="text-xs font-black bg-amber-950/10 px-2 py-1 rounded-lg text-amber-950 font-sans">
                                                                        {personalPct}%
                                                                    </span>
                                                                </div>

                                                                <div className="w-full h-3.5 bg-amber-950/10 rounded-full overflow-hidden p-0.5 mb-2.5">
                                                                    <div 
                                                                        className="h-full bg-orange-500 rounded-full transition-all duration-1000"
                                                                        style={{ width: `${Math.min(100, Math.max(2, personalPct))}%` }}
                                                                    />
                                                                </div>
            ...
            {selectedLog && <LogDetailModal log={selectedLog} canEdit={isOwnProfile} onClose={() => setSelectedLog(null)} onUpdate={handleUpdateLog} onDelete={handleDeleteLog} onViewDiploma={setViewingDiploma} />}
