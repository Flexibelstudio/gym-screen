...
const calculateTotalDuration = (settings: TimerSettings, totalExercises: number): number => {
    ...
    switch(settings.mode) {
        ...
        case TimerMode.Tabata:
            // For Interval/Tabata, 'rounds' is the total number of work intervals.
...
export const useWorkoutTimer = (block: WorkoutBlock | null) => {
  ...
  let totalRounds: number; // This is number of laps

  if (block?.settings.mode === TimerMode.Interval || block?.settings.mode === TimerMode.Tabata) {
      // New logic: settings.rounds is the TOTAL number of work intervals
      totalWorkIntervals = block.settings.rounds;
      totalRounds = totalExercises > 0 ? Math.ceil(totalWorkIntervals / totalExercises) : totalWorkIntervals;
  } else {
      // Original logic for EMOM, etc., with a fix for 0 exercises.
      const settingsRounds = block?.settings.rounds ?? 0;
      totalRounds = settingsRounds;
      totalWorkIntervals = totalExercises > 0 ? settingsRounds * totalExercises : settingsRounds;
  }
