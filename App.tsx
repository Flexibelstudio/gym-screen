...
import { createOrganization, updateGlobalConfig, updateStudioConfig, createStudio, updateOrganization, updateOrganizationPasswords, updateOrganizationLogos, updateOrganizationPrimaryColor, updateOrganizationCustomPages, isOffline, updateStudio, deleteStudio, deleteOrganization, updateOrganizationInfoCarousel, updateOrganizationDisplayWindows, deleteImageByUrl, saveRace } from './services/firebaseService';
...
const WorkoutListScreen: React.FC<WorkoutListScreenProps> = ({ passkategori, onSelectWorkout }) => {
...
const RepsOnlyScreen: React.FC<{ block: WorkoutBlock; onFinish: () => void; onShowImage: (url: string) => void; organization: Organization | null; }> = ({ block, onFinish, onShowImage, organization }) => {
...
const TimerScreen: React.FC<TimerScreenProps> = ({ 
  ...
  const allParticipants = useMemo(() => 
    ...
    const handleRaceComplete = useCallback(async () => {
        ...
        try {
            const savedRace = await saveRace(raceToSave, organization.id);
    ...
    }, [isHyroxRace, activeWorkout, organization, startGroups, finishedParticipants, onFinish]);
...
export default function App() {
  ...
  // REMOVED: The useEffect that auto-selected the first organization.
        ...
        if (!selectedOrganization) {
            return (
                <div className="flex flex-col items-center justify-center h-[50vh] text-center p-8">
                    <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">Ingen organisation hittades</h2>
                    <p className="text-gray-600 dark:text-gray-400">Ditt konto verkar inte vara kopplat till en giltig organisation. Kontakta supporten.</p>
                </div>
            );
        }
