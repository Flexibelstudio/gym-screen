...
import { Page, Workout, WorkoutBlock, TimerMode, Exercise, TimerStatus, TimerSettings, Passkategori, Studio, MenuItem, StudioConfig, Organization, CustomPage, CustomCategoryWithPrompt, UserRole, UserData, EquipmentItem, InfoCarousel, InfoMessage, DisplayConfig } from './types';
...
import { getWorkoutsForOrganization, saveWorkout, deleteWorkout, createOrganization, updateGlobalConfig, updateStudioConfig, createStudio, updateOrganization, updateOrganizationPasswords, updateOrganizationLogos, updateOrganizationPrimaryColor, updateOrganizationCustomPages, isOffline, updateStudio, deleteStudio, deleteOrganization, updateOrganizationInfoCarousel, updateOrganizationDisplayConfig } from './services/firebaseService';
...
const MainContent: React.FC = () => {
  ...
  const [completionInfo, setCompletionInfo] = useState<{ workout: Workout, isFinal: boolean, blockTag?: string } | null>(null);
...
useEffect(() => {
  ...
  const handleSaveWorkout = async (workout: Workout, startFirstBlock?: boolean) => {
    ...
    if (savedWorkout.id === workout.id) {
        ...
        if (startFirstBlock && savedWorkout.blocks.length > 0) {
            ...
            const isAdmin = (role === 'systemowner' || role === 'organizationadmin') && !isImpersonating;
            const currentPage = history[history.length - 1];
            const cameFromBuilder = currentPage === Page.WorkoutBuilder || currentPage === Page.SimpleWorkoutBuilder;

            if (isAdmin && cameFromBuilder) {
                const adminHomePage = role === 'systemowner' ? Page.SystemOwner : Page.SuperAdmin;
                setHistory([adminHomePage]);
            } else {
  ...
  const getBoostPrompt = (difficulty: 'enkel' | 'avancerad') => {
    if (difficulty === 'enkel') {
...
*   **Coach Tips**: Ge ett kort, peppande tips till deltagarna.`;
    ...
    return `Skapa en "Avancerad WOD".

**Regler:**
*   Total träningstid: 20–30 minuter.
*   Passet **MÅSTE** struktureras i två tydliga block i "blocks"-arrayen i JSON-svaret:
    *   **Första Blocket (Styrka):** Ett tungt styrkeinslag med skivstång (om tillgänglig). För detta block, sätt **ALLTID** \`settings.mode\` till \`"Time Cap"\`. Sätt en rimlig varaktighet på ca 12-15 minuter. Använd **ALDRIG** \`"Ingen Timer"\`. Sätt taggen för detta block till "Styrka".
...
*   **Titel**: Ge passet en titel i stil med "Dagens Avancerade WOD: Styrka & Flås" eller liknande.
*   **Coach Tips**: Ge ett kort, peppande tips till deltagarna.`;
  }
  ...
  const handleSelectBoostDifficulty = async (difficulty: 'enkel' | 'avancerad') => {
    ...
    const prompt = getBoostPrompt(difficulty);
    setBoostModalOpen(false);
    setIsGeneratingBoost(true);
    
    try {
      const equipmentList = studioConfig.equipmentInventory || [];
      const generatedWorkout = await generateWorkout(prompt, equipmentList);
      const now = new Date();
  ...
  const handleTimerFinish = useCallback((isNaturalFinish: boolean = false) => {
    ...
    if (activeWorkout && activeBlock) {
        ...
        setCompletionInfo({ workout: activeWorkout, isFinal: isLastBlock, blockTag: activeBlock.tag });
    } else if (activeWorkout) {
        // Fallback for freestanding timers or other edge cases
        setCompletionInfo({ workout: activeWorkout, isFinal: true, blockTag: activeWorkout.blocks[0]?.tag });
    }
  ...
  const handleUpdateOrganizationCustomPages = async (organizationId: string, customPages: CustomPage[]) => {
    ...
    const handleUpdateOrganizationDisplayConfig = async (organizationId: string, displayConfig: Organization['displayConfig']) => {
        try {
            const updatedOrg = await updateOrganizationDisplayConfig(organizationId, displayConfig!);
            setAllOrganizations(prev => prev.map(o => (o.id === organizationId ? updatedOrg : o)));
            if (selectedOrganization?.id === organizationId) {
                selectOrganization(updatedOrg);
            }
    ...
    switch (page) {
      ...
      case Page.SuperAdmin:
        return selectedOrganization && <SuperAdminScreen 
                  ...
                  onUpdateDisplayConfig={handleUpdateOrganizationDisplayConfig}
              />;
      ...
      case Page.DisplayWindow:
        return <DisplayWindowScreen onBack={handleBack} />;
      default:
        return <HomeScreen 
            navigateTo={navigateTo} 
            onSelectWorkout={handleSelectWorkout} 
  ...
  const isFullScreenPage = page === Page.Timer || page === Page.RepsOnly || page === Page.BreathingGuide || page === Page.DisplayWindow;
  const paddingClass = isFullScreenPage ? '' : 'p-4 sm:p-6 lg:p-8';
  const musicPlayerOffset = isMusicPlayerVisible ? 'bottom-20' : 'bottom-0';

  const musicPlayerHeight = 80; // h-20 in Tailwind
  const infoBannerHeight = 256; // h-64 in Tailwind

  const mainPaddingBottom = useMemo(() => {
    let padding = 0;
    if (isMusicPlayerVisible) padding += musicPlayerHeight;
    if (isInfoBannerVisible) padding += infoBannerHeight;
    return padding;
  }, [isMusicPlayerVisible, isInfoBannerVisible]);

  const qrCodeTranslateY = useMemo(() => {
    let offset = 0;
    if (isMusicPlayerVisible) offset += musicPlayerHeight;
    if (isInfoBannerVisible) offset += infoBannerHeight;
    return offset;
  }, [isMusicPlayerVisible, isInfoBannerVisible]);
  ...
  return (
    <div className={`bg-white dark:bg-black text-gray-800 dark:text-gray-200 min-h-screen font-sans flex flex-col ${paddingClass}`}>
      ...
      {isReAuthModalOpen && (
       ...
       {isInfoBannerVisible && <InfoCarouselBanner messages={activeInfoMessages} className={musicPlayerOffset} />}
...
const TimerScreen: React.FC<{ block: WorkoutBlock; onFinish: (isNatural?: boolean) => void; onHeaderVisibilityChange: (isVisible: boolean) => void; onShowImage: (url: string) => void; }> = ({ block, onFinish, onHeaderVisibilityChange, onShowImage }) => {
  ...
  const showRoundCounter = useMemo(() => {
    if (status === TimerStatus.Idle || status === TimerStatus.Finished || totalRounds <= 1) {
      return false;
    }
    // Always show minute counter for EMOMs with more than one round
    if (block.settings.mode === TimerMode.EMOM) {
      return true;
    }
    // For Interval/Tabata, only show "Varv" counter if it's a circuit of multiple exercises
    ...
    return false;
  }, [status, totalRounds, totalExercises, block.settings.mode]);
  ...
  return (
    ...
    >
      <div 
        className={`w-full flex flex-col items-center justify-center p-6 md:p-8 transition-all duration-500 ${timerStyle.bg} ${innerAnimationClass} ${isFreestanding ? 'flex-grow' : 'max-w-5xl mx-auto flex-shrink-0 rounded-2xl min-h-[200px] md:min-h-[250px] lg:min-h-[300px]'}`}
      >
            ...
            <div className="bg-gray-100 dark:bg-slate-800 rounded-lg p-4">
              <div className="space-y-2 mt-4">
                {block.exercises.map((ex, index) => {
                  const isActive = block.followMe === true && (index === currentExerciseIndex && (status === TimerStatus.Running || status === TimerStatus.Resting));
                  return (
                    <div key={ex.id} className={`p-4 rounded-lg transition-all duration-300 ${isActive ? 'bg-primary/20 ring-2 ring-primary' : 'bg-gray-200 dark:bg-slate-900'}`}>
                      <div className="flex justify-between items-center">
                        <p className={`text-3xl font-bold ${isActive ? 'text-primary' : 'text-gray-800 dark:text-gray-200'}`}>{formatExerciseName(ex)}</p>
                        {ex.imageUrl && (
