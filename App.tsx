...
const RaceFinishAnimation: React.FC<{ winnerName?: string | null }> = ({ winnerName }) => (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-[1001] p-4 animate-fade-in-out pointer-events-none">
        <Confetti />
...
const TimerScreen: React.FC<TimerScreenProps> = ({ 
  ...
  return (
    ...
    >
        {showFinishAnimation && <RaceFinishAnimation winnerName={winnerName} />}
              ...
              {block.followMe ? (
                ...
                <div className="space-y-4">
                  {block.exercises.map((ex) => {
                    const imageUrl = getExerciseImageUrl(ex, organization);
                    return (
                      <div key={ex.id} className="bg-gray-50 dark:bg-gray-800 rounded-lg shadow-md flex items-stretch overflow-hidden">
                        <div className={`w-[5px] flex-shrink-0 ${timerStyle.bg}`}></div>
                        <div className="px-5 py-3 flex-grow flex justify-between items-center min-w-0">
                            <div className="min-w-0">
                                <p className="text-xl font-semibold text-gray-900 dark:text-white">{formatExerciseName(ex)}</p>
                                {ex.description && (
