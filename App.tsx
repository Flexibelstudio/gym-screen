...
import { Page, Workout, WorkoutBlock, TimerMode, Exercise, TimerStatus, TimerSettings, Passkategori, Studio, MenuItem, StudioConfig, Organization, CustomPage, CustomCategoryWithPrompt, UserRole, UserData, EquipmentItem, InfoCarousel, InfoMessage, DisplayWindow, Note } from './types';
...
import { getWorkoutsForOrganization, saveWorkout, deleteWorkout, createOrganization, updateGlobalConfig, updateStudioConfig, createStudio, updateOrganization, updateOrganizationPasswords, updateOrganizationLogos, updateOrganizationPrimaryColor, updateOrganizationCustomPages, isOffline, updateStudio, deleteStudio, deleteOrganization, updateOrganizationInfoCarousel, updateOrganizationDisplayWindows } from './services/firebaseService';
...
const useBouncingElement = (containerRef: React.RefObject<HTMLElement>) => {
    const [position, setPosition] = useState({ x: 50, y: 50 });
    const velocityRef = useRef({ vx: 0.5, vy: 0.5 });
    ...
    useEffect(() => {
        ...
        velocityRef.current = {
            ...
            vy: (Math.random() > 0.5 ? 1 : -1) * (0.02 + Math.random() * 0.03),
        };
    }, []);
        ...
        const animate = () => {
            if (containerRef.current && elementRef.current) {
                ...
                setPosition(prevPos => {
                    ...
                    if (newX <= 0 || newX >= 100 - elementWidthPercent) {
                        velocityRef.current.vx *= -1;
                    }
                    if (newY <= 0 || newY >= 100 - elementHeightPercent) {
...
const Screensaver: React.FC<ScreensaverProps> = ({ logoUrl }) => {
    ...
    const { position: clockPosition, elementRef: clockRef } = useBouncingElement(containerRef);
    const { position: logoPosition, elementRef: logoRef } = useBouncingElement(containerRef);
...
const MainContent: React.FC = () => {
  ...
  const inactivityTimerRef = useRef<number | null>(null);
    ...
    }, [studioConfig.enableScreensaver, page]);
...
const Header: React.FC<HeaderProps> = ({ page, onBack, theme, toggleTheme, isVisible = true, activeCustomPageTitle, onSignOut, role, historyLength, showClock }) => {
...
const TimerScreen: React.FC<{ block: WorkoutBlock; onFinish: (isNatural?: boolean) => void; onHeaderVisibilityChange: (isVisible: boolean) => void; onShowImage: (url: string) => void; }> = ({ block, onFinish, onHeaderVisibilityChange, onShowImage }) => {
  ...
  return (
    ...
    >
      <div 
        className={`w-full flex flex-col items-center justify-center p-6 md:p-8 transition-all duration-500 ${timerStyle.bg} ${innerAnimationClass} ${isFreestanding ? 'flex-grow' : 'mt-4 max-w-5xl mx-auto flex-shrink-0 rounded-2xl min-h-[200px] md:min-h-[250px] lg:min-h-[300px]'}`}
      >
            ...
            <>
              ...
              {(block.settings.mode === TimerMode.Interval || block.settings.mode === TimerMode.Tabata) &&
                status !== TimerStatus.Idle && status !== TimerStatus.Finished && (
                  <>
                    <p className="text-xl lg:text-2xl">
                      {totalWorkIntervals} x ({formatTime(block.settings.workTime)} / {formatTime(block.settings.restTime)})
                    </p>
                    <p className="text-2xl lg:text-3xl">
              ...
              {/* Keep EMOM counter as it's not exercise specific */}
              {(block.settings.mode === TimerMode.EMOM && totalRounds > 1) &&
                status !== TimerStatus.Idle && status !== TimerStatus.Finished && (
                <p className="text-2xl lg:text-3xl">
                    Minut: {currentRound} / {totalRounds}
                </p>
              )}
            </>
          )}
            ...
            <div className="bg-gray-100 dark:bg-slate-800 rounded-lg p-4">
              <div className="space-y-2 mt-4">
                {block.exercises.map((ex, index) => {
                  const isActive = block.followMe === true && (index === currentExerciseIndex && (status === TimerStatus.Running || status === TimerStatus.Resting || status === TimerStatus.Preparing));
                  return (
                    <div key={ex.id} className={`p-4 rounded-lg transition-all duration-300 ${isActive ? `${timerStyle.activeHighlight.bg} ring-2 ${timerStyle.activeHighlight.ring}` : 'bg-gray-200 dark:bg-slate-900'}`}>
                      <div className="flex justify-between items-center">
                        <p className={`text-3xl font-bold ${isActive ? timerStyle.activeHighlight.text : 'text-gray-800 dark:text-gray-200'}`}>{formatExerciseName(ex)}</p>
                        {ex.imageUrl && (
