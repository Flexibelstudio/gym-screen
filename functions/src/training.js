...
const { onDocumentUpdated, onDocumentDeleted, onDocumentCreated, onDocumentWritten } = require("firebase-functions/v2/firestore");
...
const flexUpdateUserRole = onCall({
 ...
 */
const receiveExternalWorkout = onRequest(async (req, res) => {
...
async function updateStripeCoachCount(organizationId) {
  ...
  try {
    ...
    console.error(`Kunde inte uppdatera Stripe-prenumeration for org ${organizationId}:`, error);
  }
}
...
async function updateStripeScreenCount(organizationId) {
  ...
  const extraScreens = Math.max(0, activeScreens - 1);
...
const onUserUpdated = onDocumentUpdated({
  document: "users/{userId}",
  secrets: ["STRIPE_SECRET_KEY", "STRIPE_COACH_FEE_PRICE_ID", "STRIPE_SCREEN_FEE_PRICE_ID"]
}, async (event) => {
...
const onUserDeleted = onDocumentDeleted({
  document: "users/{userId}",
  secrets: ["STRIPE_SECRET_KEY", "STRIPE_COACH_FEE_PRICE_ID", "STRIPE_SCREEN_FEE_PRICE_ID"]
}, async (event) => {
...
const onWorkoutUpdated = onDocumentUpdated({
  document: "workouts/{workoutId}"
}, async (event) => {
  const beforeData = event.data.before.data();
  const afterData = event.data.after.data();
