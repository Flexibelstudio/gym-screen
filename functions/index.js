...
/* =============================================================================
 * BOKNINGSSYSTEM – Cloud Functions, batch 1 (kärna)
 * -----------------------------------------------------------------------------
 * SÅ GÖR DU:
 * Klistra in HELA det här blocket längst NER i SmartStudios functions/index.js
 * (precis före filens slut). Det återanvänder onCall, HttpsError och admin som
 * redan importeras högst upp i index.js – lägg inte till nya require-rader.
 * Deploya sedan: firebase use staging  →  firebase deploy --only functions
 *
 * Innehåller: getCallerData + isStaffRole (helpers), createBooking, cancelBooking.
 * Collection-namn: slots, services, bookings, passes, client_memberships,
 * org_settings (org_settings-doc = organizationId).
 * ========================================================================== */
...
/* =============================================================================
 * BOKNINGSSYSTEM – Cloud Functions, batch 2 (incheckning)
 * -----------------------------------------------------------------------------
 * Klistra in längst NER i functions/index.js, UNDER batch 1.
 * Återanvänder getCallerData + isStaffRole från batch 1 – definiera dem INTE igen.
 * Deploya: firebase use staging  →  firebase deploy --only functions
 *
 * Innehåller: adminCheckInBooking, getSlotDetailsForCheckIn, selfCheckInByEmail.
 * getSlotDetailsForCheckIn och selfCheckInByEmail kräver INTE inloggning
 * (självbetjäningsterminal). Övervägande: lägg App Check på dem senare.
 * ========================================================================== */

// --- FUNKTION: Personal checkar in / markerar no-show ---
exports.adminCheckInBooking = onCall(async (request) => {
  const db = admin.firestore();
  const caller = await getCallerData(request.auth);
...
exports.getSlotDetailsForCheckIn = onCall(async (request) => {
  const db = admin.firestore();
...
exports.selfCheckInByEmail = onCall(async (request) => {
  const db = admin.firestore();
...
/* =============================================================================
 * BOKNINGSSYSTEM – Cloud Functions, batch 3 (köp/"betalning")
 * -----------------------------------------------------------------------------
 * Klistra in längst NER i functions/index.js, UNDER batch 1 & 2.
 * Återanvänder getCallerData från batch 1 – definiera den INTE igen.
 * Deploya: firebase use staging  →  firebase deploy --only functions
 *
 * VIKTIGT – detta tar INTE riktig kortbetalning ännu (precis som appen idag):
 *   - Köpen skapar posterna och markeras som betalda.
 *   - Kvar att göra före skarp drift (separat steg):
 *       1) Koppla riktig Stripe-betalning (återanvänd er create-member-checkout /
 *          Connect) för purchaseMembership + purchasePass + kiosk 'stripe'.
 *       2) purchasePass: validera passType mot en riktig produkt i databasen –
 *          lita inte på klientens totalKlipp/validityMonths (se varning nedan).
 * Det som ÄR server-säkrat här: kreditkontroll (kiosk), atomiskt lageravdrag,
 * och att gamla medlemskap avslutas.
 * ========================================================================== */

// --- FUNKTION: Köp/teckna medlemskap ---
exports.purchaseMembership = onCall(async (request) => {
  const db = admin.firestore();
  const caller = await getCallerData(request.auth);
...
// ⚠ HÄRDNING FÖRE PROD: validera passType mot en riktig produkt i databasen och
//   koppla riktig betalning. Just nu litar den (som appen) på klientens uppgifter.
exports.purchasePass = onCall(async (request) => {
  const db = admin.firestore();
  const caller = await getCallerData(request.auth);
...
exports.createKioskOrder = onCall(async (request) => {
  const db = admin.firestore();
  const caller = await getCallerData(request.auth);
