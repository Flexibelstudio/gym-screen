...
const AIGenerationOverlay: React.FC = () => {
    ...
    return (
        <motion.div 
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[1000] flex flex-col items-center justify-center bg-gray-900/90 backdrop-blur-xl p-6 text-center"
        >
            <div className="relative mb-8">
                <div className="w-24 h-24 rounded-full border-4 border-indigo-500/30 animate-spin-slow"></div>
                <div className="absolute inset-0 flex items-center justify-center">
                    <div className="w-16 h-16 bg-indigo-600 rounded-full flex items-center justify-center shadow-[0_0_30px_rgba(79,70,229,0.5)] animate-pulse">
                        <SparklesIcon className="w-8 h-8 text-white" />
                    </div>
                </div>
            </div>
            
            <h2 className="text-2xl font-black text-white uppercase tracking-widest mb-2">AI-Coach Jobbar</h2>
            ...
            <motion.p 
                ...
                exit={{ opacity: 0, y: -10 }}
                className="text-indigo-200 font-medium text-sm h-6"
            >
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl p-4 mb-3 border border-gray-100 dark:border-gray-800 shadow-sm transition-all">
            <div className="flex justify-between items-start mb-3">
                <div className="flex-1 min-w-0 pr-2">
                    ...
                    {lastPerformance ? (
                        <p className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider">Senast: {lastPerformance.weight}kg x {lastPerformance.reps}</p>
                    ) : (
...
export const WorkoutLogScreen = ({ 
  ...
  useEffect(() => {
    ...
    if (savedSessionRaw) {
        ...
        } catch (e) { console.warn("Failed to parse saved session", e); }
    }
